...
\begin{itemize}
    ...
    \item We train and evaluate a global CNN model on over 6{,}000 GSOD stations worldwide, achieving an AUROC of 0.844 on a held-out 2023--2024 test period and demonstrating consistent performance across eight climatically distinct regions (Section~\ref{sec:experiments}).
...
The network is trained using the AdamW optimiser~\cite{https://doi.org/10.48550/arxiv.1412.6980} with learning rate $10^{-4}$ and weight decay $10^{-4}$, and a linear learning rate schedule that decays the rate by a factor of 5 over the full training run. We use binary cross-entropy with logits loss with a positive-class weight $w^+ = N_{\mathrm{neg}} / N_{\mathrm{pos}}$ to compensate for class imbalance. Training is distributed across two NVIDIA RTX A5000 GPUs (24~GB each) using PyTorch DDP with \texttt{fp16} mixed precision~\cite{paszke2019pytorch}. Each epoch samples 5{,}000 mini-batches of size 32 drawn uniformly at random from the full training set (shuffle enabled), ensuring geographically unbiased coverage. Models are selected by best validation loss over 50 epochs.
...
\begin{table}[t]
    ...
    \begin{tabular}{l|c|c|c|c}
         ...
         \midrule
         \textbf{CNN (full)} & \textbf{0.844} & \textbf{0.745} & \textbf{0.699} & \textbf{TBD} \\
         \midrule
...
We presented a deep learning framework for global extreme wind risk assessment that bridges the resolution gap between coarse climate model output and point-scale station observations. A convolutional neural network is trained to predict, for every CMIP6 grid cell, the probability that a locally extreme wind event will occur within a 28-day window, using a station-adaptive threshold that combines a location-specific 95th-percentile criterion with a 15~m/s physical lower bound. Trained on over 6{,}000 GSOD stations worldwide (2000--2020) and evaluated on a held-out 2023--2024 test period, the model achieves a global AUROC of 0.844 and Average Precision of 0.745, substantially outperforming both the climatological CMIP6 threshold baseline (AUROC\,=\,0.537, BSS\,=\,$-$0.476) and a logistic regression baseline (AUROC\,=\,0.590, BSS\,=\,$+$0.023). Performance is consistent across climatically diverse regions including Russia, sub-Saharan Africa, and South and East Asia, suggesting that the learned spatial features generalise well beyond the station-rich areas that dominate the training signal.
