...
Table~\ref{tab:metrics} isolates the contribution of each input variable group by removing one at a time from the best configuration. Three findings stand out.

\textbf{CMIP6 surface wind speed is harmful.} Adding \texttt{sfcWindmax} to the optimal model (row ``+\,sfcWindmax'') reduces AUROC by 0.004 and AP by 0.007. Because \texttt{sfcWindmax} is a grid-cell average at $1.125^\circ$ resolution, it systematically attenuates local wind extremes relative to station observations (Figure~\ref{fig:cmip6_vs_station}), introducing noise that the model cannot disentangle from the thermodynamic signal. Thermodynamic variables describing the large-scale atmospheric environment driving the storm are more informative than the model's direct, spatially-blurred wind estimate.

\textbf{Elevation is necessary for orographic correction.} Removing the static elevation channel (row ``$-$\,elevation'') leaves AUROC nearly unchanged (0.847) but degrades AP, reflecting reduced ability to resolve wind amplification by complex terrain at station scale. Because CMIP6 cannot represent sub-grid orographic effects at $1.125^\circ$, the DEM channel provides the only topographic context available to the model.

\textbf{Temperature gradients and precipitation both contribute.} Removing \texttt{tasmax} and \texttt{tasmin} together (row ``$-$\,temperature'') degrades AP by 0.017 (0.757\,$\to$\,0.740) and BSS by 0.020, consistent with temperature gradients being the primary proxy for baroclinic instability. Removing \texttt{pr} alone (row ``$-$\,precipitation'') produces a comparable drop \textcolor{brown}{[final run in progress]}, indicating that the two variable groups carry partially redundant but complementary information about synoptic storm-favourable conditions. Crucially, \emph{neither} group alone fully substitutes for the other, motivating the four-variable design.
...
We presented a deep learning framework for global extreme wind risk assessment that bridges the resolution gap between coarse CMIP6 climate model output and point-scale station observations. GhostWindNet27, a GhostNetV2-based CNN (11.3M parameters), is trained to predict, for every CMIP6 grid cell, the probability that a locally extreme wind event will occur within a 28-day window, using a station-adaptive threshold that combines a location-specific 95th-percentile criterion with a 15\,m/s physical lower bound.
...
A systematic ablation study reveals a counter-intuitive but physically interpretable variable hierarchy: the CMIP6 \texttt{sfcWindmax} field---the most direct wind signal available---is the \emph{least} useful predictor and slightly \emph{hurts} performance when added to the optimal set, because grid-cell averaging at $1.125^\circ$ attenuates local wind extremes. By contrast, thermodynamic variables (\texttt{tasmax}, \texttt{tasmin}) and precipitation (\texttt{pr}) capture the baroclinic instability and convective conditions that \emph{drive} extreme winds, making them far more informative. Elevation is the third essential ingredient, providing the sub-grid topographic context that CMIP6 cannot resolve. This finding has practical implications for feature selection in any statistical downscaling or extreme-event model trained on coarse climate model output.

Regional and seasonal analysis confirm that the learned representations generalise beyond the training regime. AUROC remains above 0.74 across all eight evaluated sub-regions spanning Russia and Africa, with Western Siberia (0.861) and Russia--Europe (0.841) performing best. Seasonally, the model performs strongest in DJF (AUROC\,=\,0.869, AP\,=\,0.793), tracking the annual cycle of extratropical cyclone intensity, and weakest in JJA (AUROC\,=\,0.819, AP\,=\,0.678) where convective storms dominate.

Because the model consumes CMIP6 fields as input, it generalises directly to any CMIP6 scenario---historical, near-term, or long-range projection under SSP2-4.5, SSP3-7.0, or SSP5-8.5---producing global storm-risk probability maps at $1.125^\circ$ resolution without retraining. Practical applications include identifying high-risk corridors for infrastructure siting, informing wind-energy resource assessment under climate change, and providing probabilistic hazard layers for national adaptation plans.
