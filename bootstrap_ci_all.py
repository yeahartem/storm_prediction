...
def bootstrap_ci(y_true, y_score, n_boot=2000, alpha=0.05, seed=42):
    ...
    aurocs = np.array(aurocs); aps = np.array(aps); briers = np.array(briers)
...
def main():
    ...
    for f in files:
        ...
        if "score" in df.columns and "binary_target" in df.columns:
            ...
            # Legacy: raw float / raw m/s. Fall back: binarize target at 15 m/s and use prediction as score.
            print(f"  [legacy CSV] {f}: using prediction/target with threshold 15 m/s")
        ...
        r = bootstrap_ci(y_true, y_score, n_boot=args.n_boot, seed=args.seed)
