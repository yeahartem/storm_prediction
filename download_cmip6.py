...
def find_file(session, experiment, variable, time_filter):
    """Query ESGF for the target file, prefer non-Globus URLs."""
    params = {
        "project": "CMIP6",
    ...
    for doc in docs:
        ...
        size = doc.get("size", 0)
        urls = doc.get("url", [])
        # Prefer standard HTTP/HTTPS over Globus
        http_urls = [u.split("|")[0] for u in urls
                     if "http" in u.split("|")[-1].lower()
                     and "globus" not in u.split("|")[0]
                     and "diasjp" not in u.split("|")[0]]
        all_urls  = [u.split("|")[0] for u in urls if "http" in u.split("|")[-1].lower()]
        chosen = (http_urls or all_urls)
        if chosen:
            return title, chosen[0], size
    return None, None, 0
...
def main():
    ...
    for experiment, variable, time_filter in TARGETS:
        ...
        title, url, size = find_file(session, experiment, variable, time_filter)
        if not title:
