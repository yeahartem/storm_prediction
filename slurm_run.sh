...
singularity run --bind $(pwd)/Wind:/app/wind --bind $(pwd)/data:/app/wind/data_mounted --pwd "/app/wind" docker://teshbek/xtr_weather:wind_dev116 pip3 install --upgrade pip && pip3 install torch==1.13.1+cu116 torchvision==0.14.1+cu116 --extra-index-url https://download.pytorch.org/whl/cu116 && pip3 install -r $(pwd)/Wind/environments/pytorch1.13.1_cuda116/requirements_nv.txt && python3 Wind/src/regression/train.py
