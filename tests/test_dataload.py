...
def test_climatedata_to_patches():
    ...
    for i in range(cfg.half_side_size, len(lat_coords) - cfg.half_side_size):
        for j in range(cfg.half_side_size, len(lon_coords) - cfg.half_side_size):
            for t in range(cfg.time_window//2, len(time_coords) - cfg.time_window//2):
