...
def test_naming():
    ...
    N_path_members = random.randint(1, 10)
    ...
    assert (file.variable_name == random_var_name.split('_')[0])
    assert (file.start_date == random_dates.split('-')[0]) and (file.end_date == random_dates.split('-')[1]), "dates are read incorrectly"
    assert (file.start_year == int(random_dates.split('-')[0][:4])) and (file.end_year == int(random_dates.split('-')[1][:4])), "years are read incorrectly"
