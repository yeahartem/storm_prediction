...
def climate_to_npy(files: list, var: str, cfg, save: bool = True):
    ...
    if save:
        ...
        if cfg.process.saved_normalized:
            ...
            np.save(os.path.join(cfg.data.data_dir, var + f"_{cfg.process.precision}.npy"), data.astype(dtype))
        else:
            np.save(os.path.join(cfg.data.data_dir, var + f"_{cfg.process.precision}.npy"), data_arr[var].data.astype(dtype))
        time = data_arr[var]["time"].to_numpy()       
        lat = data_arr[var]["lat"].to_numpy()
        lon = data_arr[var]["lon"].to_numpy()
        np.save(os.path.join(cfg.data.data_dir, "time.npy"), time)
        np.save(os.path.join(cfg.data.data_dir, "lat.npy"), lat)
        np.save(os.path.join(cfg.data.data_dir, "lon.npy"), lon)
...
def elevation_to_npy(file: str, cfg, save: bool = True):
    ...
    if save:
        ...
        lat = data_arr[var]["lat"].to_numpy()
        lon = data_arr[var]["lon"].to_numpy()
        np.save(os.path.join(cfg.data.data_dir, "elev_lat.npy"), lat)
        np.save(os.path.join(cfg.data.data_dir, "elev_lon.npy"), lon)
...
def save_normalization_values(mean_channels: np.array, std_channels: np.array, cfg: DictConfig, prefix: str = ''):
    ...
    np.save(os.path.join(cfg.data.data_dir, prefix + "mean_32.npy"), mean_channels.astype(np.float32))
    np.save(os.path.join(cfg.data.data_dir, prefix + "std_32.npy"), std_channels.astype(np.float32))
    np.save(os.path.join(cfg.data.data_dir, prefix + "mean_16.npy"), mean_channels.astype(np.float16))
    np.save(os.path.join(cfg.data.data_dir, prefix + "std_16.npy"), std_channels.astype(np.float16))
...
def prepare_cmip(cfg: DictConfig):    
    ...
    with open_dict(cfg):
        cfg.data.path_to_prepared_target_data = os.path.join(cfg.data.data_dir, "target.parquet")
        cfg.data.path_to_prepared_stations = os.path.join(cfg.data.data_dir, "stations.parquet")
        cfg.raw.path_to_weather_stations_data = os.path.join(cfg.raw.path_to_weather_stations, "data_meteo_full.parquet")
        cfg.path_to_weather_station_list = os.path.join(cfg.raw.path_to_weather_stations, "weatherstation_list.json")
        cfg.path_to_world_weather_stations_data = os.path.join(cfg.raw.path_to_weather_stations, "world_stations_25_days_6_months.parquet")
    ...
    if cfg.process.make_climate_data:
        for folder in cfg.raw.paths_to_climate_files_folders:
            for var in cfg.process.variables:
                ...
                logging.info(f"{var} data saved to {cfg.data.data_dir}")
    #elevation
    if cfg.process.make_elevation_data:
        ...
        logging.info(f"elevation data saved to {cfg.data.data_dir}")
    ...
    if cfg.process.make_normalization and cfg.process.make_climate_data:
        ...
        logging.info(f"Normalization values saved to {cfg.data.data_dir}")
    ...
    if cfg.process.make_target:
        ...
        logging.info(f"Target data saved to {cfg.data.data_dir} as {cfg.raw.prepared_target_data_name}")

    with open(os.path.join(cfg.data.data_dir, 'dataset_config.yaml'), 'w') as file:
