...
class EvalDataset(torch.utils.data.Dataset):
    ...
    def load_dataset(self):
        ...
        self.time_coords_full = np.load(os.path.join(self.cfg.data_dir, 'time.npy')).astype('datetime64[D]')
        self.lat_coords_full = np.load(os.path.join(self.cfg.data_dir, 'lat.npy'))
        self.lon_coords_full = np.load(os.path.join(self.cfg.data_dir, 'lon.npy'))
        self.var_data = np.empty((len(self.cfg.variables), len(self.time_coords_full), len(self.lat_coords_full), len(self.lon_coords_full)), dtype=np.float16)
        for i, var in enumerate(self.cfg.variables):
            self.var_data[i] = np.load(os.path.join(self.cfg.data_dir, var + f'_{self.cfg.precision}.npy'))

    def limit_inference_space(self):
        """Crop data by spatial coordinates"""
        time_start = pd.to_datetime(self.cfg.time_start)
        time_end = pd.to_datetime(self.cfg.time_end)
        self.lat_min_idx = np.searchsorted(self.lat_coords_full, self.cfg.lat_min)
        self.lat_max_idx = np.searchsorted(self.lat_coords_full, self.cfg.lat_max)
        self.lon_min_idx = np.searchsorted(self.lon_coords_full, self.cfg.lon_min)
        self.lon_max_idx = np.searchsorted(self.lon_coords_full, self.cfg.lon_max) 
...
def eval(cfg: DictConfig) -> None:        
    ...
    result_df = predict(model, dataset, batch_size=cfg.batch_size, distributed=cfg.distributed)
