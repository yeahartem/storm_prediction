...
class WindNetPL(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        lat = (pos[:, 0, 2] * 90).cpu()
        lon = (pos[:, 0, 3] * 180).cpu()
        month = (pos[:, 0, 1] * 12).round().long().cpu()  # 1-12
    ...
    def on_test_epoch_end(self):
        ...
        for season_name, season_months in SEASONS.items():
            ...
            seasonal_rows.append({"season": season_name, "AUROC": auroc, "AP": ap, "F1": f1_val,
                                  "pos_rate": pos_rate, "n_samples": n})
        if seasonal_rows:
            seas_df = pd.DataFrame(seasonal_rows)
        ...
        disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=["No storm", "Storm"])
